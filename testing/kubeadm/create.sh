...
first_node() {
  ...
  logr "INFO" "$(ssh $NODE01  "docker rmi ghcr.io/rid/kube-vip-leaseweb:$kube_vip_version" 2>&1)"

  # echo "echo "ip_vs | tee -a /etc/modules"
  logr "INFO" "Creating Kube-vip.io Manifest" 
  ssh $NODE01 "sudo docker run --network host --rm ghcr.io/rid/kube-vip-leaseweb:$kube_vip_version manifest pod  --interface ens160 --vip $kube_vip_vip --arp --leaderElection --enableLoadBalancer $kube_vip_mode | sed  \"s/image:.*/image: plndr\/kube-vip:$kube_vip_version/\" | sudo tee /etc/kubernetes/manifests/vip.yaml" >> $logfile
...
additional_controlplane() {
  ...
  sleep 1
  ssh $NODE02 "sudo docker run --network host --rm ghcr.io/rid/kube-vip-leaseweb:$kube_vip_version manifest pod --interface ens160 --vip $kube_vip_vip --arp --leaderElection --enableLoadBalancer $kube_vip_mode | sed  \"s/image:.*/image: plndr\/kube-vip:$kube_vip_version/\"| sudo tee /etc/kubernetes/manifests/vip.yaml" >> $logfile
  logr "INFO" "Adding $NODE03"
  ssh $NODE03 "sudo $JOIN_CMD $CONTROLPLANE_CMD" >> $logfile
  sleep 1
  ssh $NODE03 "sudo docker run --network host --rm ghcr.io/rid/kube-vip-leaseweb:$kube_vip_version manifest pod --interface ens160 --vip $kube_vip_vip --arp --leaderElection --enableLoadBalancer $kube_vip_mode | sed  \"s/image:.*/image: plndr\/kube-vip:$kube_vip_version/\"| sudo tee /etc/kubernetes/manifests/vip.yaml" >> $logfile
}
